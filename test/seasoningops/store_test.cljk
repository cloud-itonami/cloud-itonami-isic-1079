...
(deftest production-batch-test
  (testing "retrieve an existing batch"
    (let [batch-data {:product-type :seasoning/instant-dashi-powder :moisture-content-percent 3.5}
...
(deftest log-batch-test
  (testing "logging a batch marks it as processed"
    (let [st {:batches {}}
          batch-data {:product-type :seasoning/instant-dashi-powder}
  ...
  (testing "logging preserves batch data"
    (let [st {:batches {}}
          batch-data {:product-type :seasoning/instant-dashi-powder :moisture-content-percent 3.5}
          result (store/log-batch st "batch-001" batch-data)]
      (is (= (:product-type (get-in result [:batches "batch-001"])) :seasoning/instant-dashi-powder))
      (is (= (:moisture-content-percent (get-in result [:batches "batch-001"])) 3.5)))))

;; ──────────────────────── Shipment Finalization ──────────────────────

(deftest finalize-shipment-test
  (testing "finalizing a batch marks it as finalized"
    (let [st {:batches {"batch-001" {:product-type :seasoning/instant-dashi-powder}}}
