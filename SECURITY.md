...
If you discover a security issue in cloud-itonami-isic-1079, **do not open a
...
This repository implements the **Governor** for instant-seasoning/soup-mix
(dry-mix blending and packaging) plant operations — a deterministic
safety-checking layer that gates all proposals from the LLM/advisor.

Security-critical concerns:
1. **Hard violations in Governor rules must never be overridable** — a rule
   marked as HARD (e.g., `:already-processed`, `:moisture-content-exceeds-max`,
   `:microbial-load-exceeds-max`, `:foreign-material-detected`,
